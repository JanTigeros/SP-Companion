...
$fullPrice = 0;
  ...
  function shraniRacunVBazo($kraj_izdaje, $datum_storitve, $datum_izdaje, $datum_zapada, $productList, $company, $conn, $fullPrice) {

    // Priprava SQL poizvedbe za vstavljanje podatkov
    $stmt = $conn->prepare("INSERT INTO racun (id_racun, kraj_izdaje, datum_storitve, datum_izdaje, datum_zapada, podjetje_id, vsota) VALUES (null, :kraj_izdaje, :datum_storitve, :datum_izdaje, :datum_zapada, :podjetje_id, :vsota)");
    // Izvedba poizvedbe
    $stmt->execute(array(':kraj_izdaje' => $kraj_izdaje, ':datum_storitve' => $datum_storitve, ':datum_izdaje' => $datum_izdaje, ':datum_zapada' => $datum_zapada, ':podjetje_id' => $company, ':vsota' => $fullPrice));
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["invoice_name"]) && isset($_POST["invoice-date-service"]) && isset($_POST["invoice-date"]) && isset($_POST["invoice-expire"]) && isset($_POST["productList"]) && isset($_POST["company"])) {
        ...
        $fullPrice = shraniRacunVBazo($kraj_izdaje, $datum_storitve, $datum_izdaje, $datum_zapada, $productList, $company, $conn, $fullPrice);
    } else {
...
<body>
    ...
    <div id="content">
        <div class="invoice">
            ...
            <hr>
            <?php 
                $sql3 = $conn->prepare("SELECT * FROM podjetje p INNER JOIN kraj k ON p.kraj_id = k.id_kraj WHERE id_podjetje = $company");
                $sql3->execute();
                $result = $sql3->fetch(PDO::FETCH_ASSOC);
            ?>
            <p><?php echo $result['naziv_podjetja'] ?></p>
            <p><?php echo $result['naslov_podjetja'] ?></p>
            <p><?php echo $result['posta'] . " " . $result['ime_kraja'] ?></p>
            <hr>
            <?php
